...
function buildBaseUrl(request: EntryPoints.Suitelet.onRequestContext["request"], lang: support.Lang): string {
  // Strip query string; re-attach only the `script` and `deploy` params
  // that NetSuite requires on all Suitelet URLs, plus the selected language so
  // every internal link (form action, tickets/agenda, back links) keeps it.
  ...
  return support.withLang(`${path}?script=${script}&deploy=${deploy}`, lang);
}
...
export function onRequest(context: EntryPoints.Suitelet.onRequestContext): void {
  ...
  const lang    = support.normLang(params.lang);
  const base    = buildBaseUrl(request, lang);

  if (request.method === "GET" && params.view === "tickets") {
    response.write(support.renderTicketsPage({ tickets: loadTickets(), baseUrl: base, lang }));
    return;
  }

  if (request.method === "GET" && params.view === "agenda") {
    response.write(support.renderAgendaPage({ baseUrl: base, lang }));
    return;
  }
  ...
  response.write(support.renderPage({ baseUrl: base, recentTickets: loadTickets(RECENT_TICKETS_LIMIT), lang }));
}
