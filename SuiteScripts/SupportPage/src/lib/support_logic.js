...
define(["require", "exports"], function (require, exports) {
    ...
    function renderTicketsPage(state = {}) {
        ...
        const filterBar = tickets.length === 0 ? "" : [
            ...
            '          <label for="flt-q">Search</label>',
            '          <input id="flt-q" type="text" placeholder="ID, name, email or topic…" autocomplete="off">',
            "        </div>",
            '        <div class="filter-field">',
            '          <label for="flt-topic">Topic</label>',
            `          <select id="flt-topic"><option value="">All topics</option>${topicOptions}</select>`,
            "        </div>",
            '        <div class="filter-field">',
            ...
            "        </div>",
            '        <button id="flt-clear" type="button" class="btn-secondary">Clear</button>',
            "      </div>",
            '      <div class="filter-meta" id="flt-meta"></div>'
        ].join("\n");
        const noMatchRow = tickets.length === 0 ? "" :
            '<tr id="no-match-row" style="display:none;"><td colspan="5" class="empty">No tickets match your filters.</td></tr>';
        const body = [
            '    <section class="card">',
        ...
        const filterScript = tickets.length === 0 ? "" : [
            ...
            "        if (meta) meta.textContent = 'Showing ' + shown + ' of ' + total + ' ticket' + (total === 1 ? '' : 's');",
            "      }",
