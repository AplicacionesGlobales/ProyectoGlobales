...
export class LandingDataService {
  ...
  async getDashboardMetrics(): Promise<BaseResponseDto<DashboardMetricsDto>> {
    try {
      ...
      // Business metrics
      const growthRate =
        newBrandsLastMonth > 0
          ? ((newBrandsThisMonth - newBrandsLastMonth) / newBrandsLastMonth) *
            100
          : 0;

      const businessTypeDistribution: BusinessTypeDistribution[] =
        brandsByBusinessType.map((item) => ({
      ...
      const appointmentStatusCounts: AppointmentStatusCount[] =
      ...
      const appointmentMetrics: AppointmentMetrics = {
        ...
        averagePerBrand:
          activeBrands > 0
            ? Math.round((totalAppointmentsCount / activeBrands) * 100) / 100
            : 0,
        totalRevenue: Number(appointmentRevenue._sum.price) || 0,
      };

      // Revenue metrics
      const totalSubscriptionAmount =
      ...
      const paymentDistribution: PaymentStatusDistribution[] =
        paymentsByStatus.map((item) => ({
          ...
          percentage:
            totalSubscriptionAmount > 0
              ? ((Number(item._sum.amount) || 0) / totalSubscriptionAmount) *
                100
              : 0,
        }));
      ...
      const revenueMetrics: RevenueMetrics = {
        ...
        averageRevenuePerBrand:
          activeBrands > 0
            ? Math.round((totalSubscriptionAmount / activeBrands) * 100) / 100
            : 0,
      };

      // User metrics
      const clientsWithMultiple = clientAppointmentStats.filter(
        (stat) => stat._count.clientId > 1,
      ).length;
      const totalClientAppointments = clientAppointmentStats.reduce(
        (sum, stat) => sum + stat._count.clientId,
        0,
      );
      const avgAppointmentsPerClient =
        clientAppointmentStats.length > 0
      ...
      const userMetrics: UserMetrics = {
        ...
        averageAppointmentsPerClient:
          Math.round(avgAppointmentsPerClient * 100) / 100,
        clientRetentionRate:
          totalClients > 0 ? (clientsWithMultiple / totalClients) * 100 : 0,
      };
  ...
  private async getDaily30DaysAppointments(
    startDate: Date,
  ): Promise<DailyMetric[]> {
    try {
    ...
    try {
      ...
      payments.forEach((payment) => {
        if (payment.processedAt) {
          ...
          dailyRevenue.set(
            date,
            (dailyRevenue.get(date) || 0) + Number(payment.amount),
          );
        }
      });
  ...
  private async getMonthlyStats(startDate: Date): Promise<MonthlyMetric[]> {
    try {
      ...
      const monthlyData = new Map<
        string,
        { brands: number; appointments: number; revenue: number }
      >();
  ...
  async getBrandDashboardMetrics(
    brandId: number,
  ): Promise<BaseResponseDto<BrandDashboardMetricsDto>> {
    try {
      ...
      const daysSinceCreation = Math.floor(
        (now.getTime() - brand.createdAt.getTime()) / (1000 * 60 * 60 * 24),
      );
      ...
      const appointmentStatusCounts: AppointmentStatusCount[] =
        appointmentsByStatus.map((item) => ({
          ...
          percentage:
            totalAppointments > 0
              ? (item._count.status / totalAppointments) * 100
              : 0,
        }));
      ...
      const averagePerAppointment =
        totalAppointments > 0 ? totalRevenueAmount / totalAppointments : 0;
